<template>
  <v-container>
    <v-breadcrumbs class="pt-0" large>ລາຍງານຍອດຂາຍ</v-breadcrumbs>
    <v-row class="mb-n6">
      <v-col>
        <v-row>
          <v-col>
        ...
        <v-row>
          ...
          <v-col>
            <SaleAdmin v-model="created_by" :firstSale="created_by" label="ເລືອກຜູ້ສ້າງ"></SaleAdmin>
          </v-col>

          <v-col>
            <v-btn color="green" dark style="width:100%" @click="fetchData()"
            >ຄົ້ນຫາ
            </v-btn
            >
          </v-col>
    ...
    <v-row>
      <v-col>
        <v-btn color="green" dark style="width:100%" @click="fetchData(true)"
        >Export
        </v-btn>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col>
        <v-row>
          <v-col>
            <v-card>
              <v-card-title v-if="pagination.total">{{ Intl.NumberFormat().format(pagination.total) }} ລູກຄ້າ
              </v-card-title>
              <v-card-text>
                <v-simple-table
                  ...
                  <template v-for="customer in customers">
                    <tr :key="customer.id">
                      <td :rowspan="customer.user.billings && customer.user.billings.length ? customer.user.billings.length : 1">
                        <v-row>
                          <a class="font-weight-bold text--accent-3 ml-6" href="#"
                        ...
                        <v-row>
                          <v-icon class="ml-8 mr-1" small> mdi-phone</v-icon>
                          {{ customer.user.phone }}
                        </v-row>
                      ...
                      <td :rowspan="customer.user.billings && customer.user.billings.length ? customer.user.billings.length : 1">
                        <v-icon> {{ customer.customer_type == 'home' ? 'mdi-account-group' : 'mdi-office-building' }}
                        </v-icon>
                        {{ customer.cost_by_la }}
                      </td>
                      <td :rowspan="customer.user.billings && customer.user.billings.length ? customer.user.billings.length : 1">
                      ...
                      <template v-if="customer.user.billings && customer.user.billings.length > 0">
                        <td>
                          <v-row>
                            <a class="font-weight-bold text--accent-3" href="#"
